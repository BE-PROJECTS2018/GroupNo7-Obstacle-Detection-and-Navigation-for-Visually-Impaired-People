...
public class CannyEdgeDetector {
    ...
    private void readLuminance(int type) {
        //int type = sourceImage.getConfig();
        ...
        if (type == 1 || type == 2) {
            ...
            // Type is gray
            int[] pixels = new int[width*height];
            sourceImage.getPixels(pixels, 0, 0, 0,0, width, height);
            for (int i = 0; i < picsize; i++) {
                data[i] = (pixels[i] & 0xff);
            }
        } else if (type == 4) {
            // Bitmap.TYPE_USHORT_GRAY
            int[] pixels = new int[width*height];
            sourceImage.getPixels(pixels, 0, 0, 0,0, width, height);
            for (int i = 0; i < picsize; i++) {
    ...
    private void writeEdges(int pixels[]) {
        ...
        edgesImage = Bitmap.createBitmap(pixels,width,height,sourceImage.getConfig());
